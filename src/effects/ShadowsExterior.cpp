...
void ShadowsExteriorEffect::RegisterTextures() {
	bool ShadowBufferPrecision = TheSettingManager->GetSettingI("Shaders.ShadowsExteriors.Main", "ShadowBufferPrecision");
	bool PointBufferRes = TheSettingManager->GetSettingI("Shaders.ShadowsExteriors.Main", "PointBufferRes");
	bool PointBufferPrecision = TheSettingManager->GetSettingI("Shaders.ShadowsExteriors.Main", "PointBufferPrecision");
	D3DFORMAT ShadowBufferType;
	D3DFORMAT PointBufferType;

	if (ShadowBufferPrecision)
		ShadowBufferType = D3DFMT_D32F_LOCKABLE;
	else
		ShadowBufferType = D3DFMT_D24S8;

	if (PointBufferPrecision) {
		PointBufferType = D3DFMT_G32R32F;
	}
	else {
		PointBufferType = D3DFMT_G16R16F;
	}
	...
	for (int i = 0; i <= MapLod; i++) {
		...
		TheRenderManager->device->CreateDepthStencilSurface(ShadowMapSize, ShadowMapSize, ShadowBufferType, D3DMULTISAMPLE_NONE, 0, true, &ShadowMaps[i].ShadowMapDepthSurface, NULL);
	...
	TheRenderManager->device->CreateDepthStencilSurface(orthoMapRes, orthoMapRes, ShadowBufferType, D3DMULTISAMPLE_NONE, 0, true, &ShadowMaps[MapOrtho].ShadowMapDepthSurface, NULL);
	...
	TheRenderManager->device->CreateDepthStencilSurface(ShadowCubeMapSize, ShadowCubeMapSize, ShadowBufferType, D3DMULTISAMPLE_NONE, 0, true, &Textures.ShadowCubeMapDepthSurface, NULL);
	...
	if (PointBufferRes)
		TheTextureManager->InitTexture("TESR_PointShadowBuffer", &Textures.ShadowPassTexture, &Textures.ShadowPassSurface, TheRenderManager->width, TheRenderManager->height, PointBufferType);
	else
		TheTextureManager->InitTexture("TESR_PointShadowBuffer", &Textures.ShadowPassTexture, &Textures.ShadowPassSurface, TheRenderManager->width / 2, TheRenderManager->height / 2, PointBufferType);
}
