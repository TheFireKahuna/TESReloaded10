...
__declspec(noinline) BSCullingProcess* GetGRTCullingProcess()
{
	...
	if (!cullingProcess)
	{
		auto cullingProc = ThisStdCall<BSCullingProcess*>(0x4A0EB0, Ni_Alloc(sizeof(BSCullingProcess) + sizeof(BSShaderAccumulator)), 0);
		auto shaderAccum = ThisStdCall<BSShaderAccumulator*>(0xB660D0, (UInt8*)cullingProc + sizeof(BSCullingProcess), 0x63, 1, 0x2F7);
