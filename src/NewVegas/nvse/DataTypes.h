...
struct NiPoint3
{
	...
	NiPoint3() {}
	__forceinline NiPoint3(float _x, float _y, float _z) : x(_x), y(_y), z(_z) {}
	__forceinline NiPoint3(const NiPoint3& rhs) { *this = rhs; }
	__forceinline explicit NiPoint3(const NiMatrix33& rhs) { *this = rhs; }
	__forceinline explicit NiPoint3(const NiQuaternion& rhs) { *this = rhs; }
	__forceinline explicit NiPoint3(const __m128 rhs) { SetPS(rhs); }

	__forceinline void operator=(NiPoint3&& rhs)
	{
		x = rhs.x;
		y = rhs.y;
		z = rhs.z;
	}
	__forceinline void operator=(const NiPoint3& rhs)
	{
		_mm_storeu_si64(this, _mm_loadu_si64(&rhs));
		z = rhs.z;
	}
	...
	__forceinline NiPoint3& operator+=(__m128 packedPS) { return SetPS(*this + packedPS); }
	__forceinline NiPoint3& operator-=(__m128 packedPS) { return SetPS(*this - packedPS); }
	__forceinline NiPoint3& operator*=(float s) { return SetPS(*this * s); }
	__forceinline NiPoint3& operator*=(__m128 packedPS) { return SetPS(*this * packedPS); }
	...
	inline bool operator==(const NiPoint3& rhs) const { return Equal_V3(PS(), rhs.PS()); }
	inline bool operator!=(const NiPoint3& rhs) const { return !(*this == rhs); }

	inline float __vectorcall DotProduct(const NiPoint3& rhs) const
	{
		__m128 k = _mm_setzero_ps();
		return _mm_cvtss_f32(_mm_hadd_ps(_mm_hadd_ps(PS3() * rhs.PS(), k), k));
	}
	...
	inline float __vectorcall Length() const { return Length_V4(PS3()); }
	NiPoint3& Normalize();
	__m128 __vectorcall CrossProduct(const NiPoint3& vB) const;
	__m128 __vectorcall Interpolate(const NiPoint3& vB, float t) const;
	__m128 __vectorcall GetTranslatedPos(const NiTransform& transfrm) const;
	__m128 __vectorcall GetRotatedPos(__m128 pry, const NiPoint3& origin) const;

	inline float __vectorcall GetAngleBetweenV3(const NiPoint3& vB) const
	{
		return ACos(DotProduct(vB));
	}
