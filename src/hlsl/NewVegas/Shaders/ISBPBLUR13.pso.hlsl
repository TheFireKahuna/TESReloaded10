...
#define	sqr(v)			((v) * (v))
    ...
    r0 = tex2D(ScreenSpace, (BlurScale.xy * BlurOffsets[12].xy) + IN.ScreenOffset.xy);
    ...
    r13 = tex2D(AvgLum, IN.ScreenOffset.xy);
    r2.xyz = max(r2.xyz - HDRParam.x, 0) * HDRParam.y;
    r0.xyz = max(r0.xyz - HDRParam.x, 0) * HDRParam.y;
    q8.xyz = (BlurOffsets[0].z * r2.xyz) + ((max(r1.xyz - HDRParam.x, 0) * HDRParam.y) * BlurOffsets[1].z);
    r2.xyz = max(r5.xyz - HDRParam.x, 0) * HDRParam.y;
    r1.xyz = (BlurOffsets[2].z * (max(r3.xyz - HDRParam.x, 0) * HDRParam.y)) + q8.xyz;
    ...
    q14.xyz = (BlurOffsets[10].z * r2.xyz) + ((BlurOffsets[9].z * (max(r10.xyz - HDRParam.x, 0) * HDRParam.y)) + r1.xyz);
    q6.xyz = (BlurOffsets[12].z * r0.xyz) + ((BlurOffsets[11].z * (max(r12.xyz - HDRParam.x, 0) * HDRParam.y)) + q14.xyz);
    OUT.color_0.a = weight(r13.xyz);
    OUT.color_0.rgb = q6.xyz;

    return OUT;
};

// approximately 82 instruction slots used (14 texture, 68 arithmetic)
