...
float3 BonesTransformPosition(float4 bones[54], float4 offset, float4 blend, float4 pos) {
    ...
    helper.x = dot(bones[offset.x], pos);
    helper.y = dot(bones[offset.x + 1], pos);
    helper.z = dot(bones[offset.x + 2], pos);
    result.xyz = helper.xyz * blend.x;
    
    helper.x = dot(bones[offset.y], pos);
    helper.y = dot(bones[offset.y + 1], pos);
    helper.z = dot(bones[offset.y + 2], pos);
    result.xyz += helper.xyz * blend.y;
    
    helper.x = dot(bones[offset.z], pos);
    helper.y = dot(bones[offset.z + 1], pos);
    helper.z = dot(bones[offset.z + 2], pos);
    result.xyz += helper.xyz * blend.z;
    
    helper.x = dot(bones[offset.w], pos);
    helper.y = dot(bones[offset.w + 1], pos);
    helper.z = dot(bones[offset.w + 2], pos);
