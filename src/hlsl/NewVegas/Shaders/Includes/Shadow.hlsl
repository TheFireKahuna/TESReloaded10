...
float GetLightAmountPP(float4 ShadowPos, float4 ShadowPosFar) {

	if (TESR_ShadowData.x != -1.0f) return 1.0f; // Shadows are not applied in post processing (ShadowsExteriors.fx.hlsl)
