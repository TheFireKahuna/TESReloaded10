...
#define	sqr(v)			((v) * (v))
    ...
    r0 = tex2D(ScreenSpace, (BlurScale.xy * BlurOffsets[10].xy) + IN.ScreenOffset.xy);
    ...
    r11 = tex2D(AvgLum, IN.ScreenOffset.xy);
    r2.xyz = max(r2.xyz - HDRParam.x, 0) * HDRParam.y;
    r0.xyz = max(r0.xyz - HDRParam.x, 0) * HDRParam.y;
    q7.xyz = (BlurOffsets[0].z * r2.xyz) + ((max(r1.xyz - HDRParam.x, 0) * HDRParam.y) * BlurOffsets[1].z);
    r2.xyz = max(r5.xyz - HDRParam.x, 0) * HDRParam.y;
    r1.xyz = (BlurOffsets[2].z * (max(r3.xyz - HDRParam.x, 0) * HDRParam.y)) + q7.xyz;
    ...
    q12.xyz = (BlurOffsets[8].z * r2.xyz) + ((BlurOffsets[7].z * (max(r8.xyz - HDRParam.x, 0) * HDRParam.y)) + r1.xyz);
    q5.xyz = (BlurOffsets[10].z * r0.xyz) + ((BlurOffsets[9].z * (max(r10.xyz - HDRParam.x, 0) * HDRParam.y)) + q12.xyz);
    OUT.color_0.a = weight(r11.xyz);
