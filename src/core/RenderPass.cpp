...
void RenderPass::RenderAccum() {
	...
	for (NiGeometry* Geo : GeometryList) {
		UpdateConstants(Geo);
		PixelShader->SetCT();
		VertexShader->SetCT();
		...
		if (GeoData && GeoData->VertCount) {
			...
			if (pStrips) {
				ThisStdCall(0xE74840, NiDX9Renderer::GetSingleton(), Geo);
			}
			else {
				ThisStdCall(0xE745A0, NiDX9Renderer::GetSingleton(), Geo);
			}
		}
	}
	GeometryList.clear();
}
void SkinnedGeoShadowRenderPass::RenderAccum() {
	if (GeometryList.empty()) return;
...
bool SkinnedGeoShadowRenderPass::AccumObject(NiGeometry* Geo) {
	...
	if (Geo->skinInstance &&
		...
		if (Geo->skinInstance->SkinPartition->Partitions[0].BuffData) GeometryList.push_back(Geo);
